...
### 완료
- chatbot-cbt 문서 페이지 배포 (`psycinno/psychology-ai/chatbot-cbt/`)
- RAG 블록 전체 수정 — 6개 블록 개정 완료
  - RAG_3: 무새 표현("그런 일이 있으셨군요") 제거, 감정 반영 표현 교체
  - RAG_4: 호흡 개입 전면 금지 처리 (1회성 CBT 쇼케이스에 부적합)
  - RAG_5: 무력감형 Stuck Point 시나리오 추가
  - RAG_6: 넛지 범용화 (특정 시나리오 의존 → 범용 행동실험 유도 구조)
  - RAG 전체: 행동실험·인지과제 혼용 금지 원칙 명시
- 학생 테스트 대화 2건 검토 (도촬 피해자 시나리오 기반)

### 발견한 문제
- 무새 동조 패턴이 실제 학생 테스트 대화에서 반복 확인됨
- 호흡 안내가 1회성 쇼케이스 맥락에서 불필요한 개입으로 작동
- 행동실험과 인지과제가 혼용될 경우 CBT 충실도 저하 위험

### 결정 사항
- 1회성 CBT 쇼케이스 구조 확정 (다회기 가정 금지)
- RAG_4 소크라테스 질문 15자 이내 단순화 — 백로그 유지 (06.02 마감 임박)
- 교수 자문 예정: CSE/PACT 척도 + Step-down 기준 임상 타당도

### 참고 자료
- `~/Downloads/rag-victim-police-bot-cbt-final.md` (최종 RAG 수정본)
- 학생 테스트 대화 2건 (도촬 피해자 시나리오)
